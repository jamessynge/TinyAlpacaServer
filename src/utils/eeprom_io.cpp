...
void Crc32::appendByte(uint8_t v) {
  TAS_VLOG(1) << "Crc32::appendByte(" << (v + 0) << ") old value=0x" << BaseHex
              << value_;
  value_ = GetCrcTableEntry(value_ ^ v) ^ (value_ >> 4);
  value_ = GetCrcTableEntry(value_ ^ (v >> 4)) ^ (value_ >> 4);
  value_ = ~value_;
  TAS_VLOG(1) << "new value=0x" << BaseHex << value_;
}
...
int Crc32::put(int crcAddress) const {
  ...
  TAS_VLOG(1) << "Crc32::put(" << crcAddress << ") value=0x" << BaseHex
              << value_;
...
bool Crc32::verify(int crcAddress) const {
  TAS_VLOG(1) << "Crc32::verify(" << crcAddress << ") computed value=0x"
              << BaseHex << value_;
  uint32_t stored = 0;
  EEPROM.get(crcAddress, stored);
  TAS_VLOG(1) << "stored value=0x" << BaseHex << stored;
