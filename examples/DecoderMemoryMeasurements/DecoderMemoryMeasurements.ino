...
// program (FLASH) and globals (RAM) storage requirements of decoding an HTTP request
// with an optional body with URL encoded parameters.
...
// Add call to reset_decoder (hence to RequestDecoder::Request): 6654/768 (i.e. +4198/+526)
// Calling Reset() likely brought in all of the DecodeFunctions because the first of them
// is referenced by Reset, and they're chained together.
// RAM rose so much because of all the literal strings that get copied into RAM
// in order to be used.
void reset_decoder() {
  decoder.Reset();
}
